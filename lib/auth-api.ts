...
const ASSET_BASE = "https://admin.meetexpertbd.xyz"
...
export function resolveAvatarUrl(avatar?: string | null) {
  ...
  if (avatar.startsWith("/")) return `${ASSET_BASE}${avatar}`
  if (avatar.startsWith("storage/")) return `${ASSET_BASE}/${avatar}`
  return `${ASSET_BASE}/storage/${avatar}`
}
